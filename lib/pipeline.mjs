...
export async function writeScript(topic, notes, level, minutes, language = "English", goal = "general understanding", angle = "") {
...
// ---------- Full pipeline ----------
export async function generateLesson({ topic, level, goal, angle, minutes, voice = "en-IN-NeerjaNeural", language = "English", onProgress = () => {} }) {
  ...
  const outputDir = path.resolve("public/out");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  await fs.writeFile("./public/out/last-script.txt", result.script);
}
